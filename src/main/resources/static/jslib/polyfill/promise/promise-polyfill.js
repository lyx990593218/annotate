...
// 简单封装一下ajax
if (!$.promiseAjax) {
    $.promiseAjax = function promiseAjax(url, paramJson, config) {
        return new Promise(function (resolve, reject) {
            var contentType = '';
            if (config && config['RequestBody']) {
            ...
            $.ajax({
                ...
                url: url,
                data: paramJson,
                dataType: config ?( config.dataType||"json"):"json",
                contentType: contentType,
                success: function (data) {
                    clearTimeout(loadIndex);
                    layer.closeAll('loading');
                    if(data == 'sessionOut'){
                        layer.confirm('您还没有登录或登录已超时，请重新登录！',
                            {
                                btn: ['确定'] //按钮
                            },
                            $.proxy(function () {
                                top.location.href = $.util.rootPath + "/jf/login";
